...
module.exports = function (forEach, t) {
	...
	t.test('skips non-existing values', function (st) {
		...
		var i = 0;
		forEach(array, function () { i += 1; });
		st.equal(i, 3, 'array');

		i = 0;
		forEach(arrayLike, function () { i += 1; });
	...
	t.test('passes the correct values to the callback', function (st) {
		...
		forEach(
			...
			function (value, key, list) {
				...
				st.equal(key, 0, 'second argument is the indvar hasStrictMode = require('has-strict-mode')();
var global = Function('return this')(); // eslint-disable-line no-new-func
var identity = function (x) { return x; };
var arrayWrap = function (x) { return [x]; };
...
module.exports = function (forEach, t) {
	...
	t.test('does not visit elements added to the array after it has begun', function (st) {
		...
		st.equal(i, 3, 'forEach callback only called thrice');

		var arrayLike = createArrayLikeFromArray([1, 2, 3]);
		i = 0;
		forEach(arrayLike, function (a) {
			i += 1;
	...
	t.test('sets the right context when given none', function (st) {
		var context;
		forEach([1], function () { context = this; });
	...
	t.test('empty array', function (st) {
		var arr = [];
		var actual = forEach(arr, identity);
		st.equal(actual, undefined, 'empty array returns undefined');
	...
	t.test('list arg boxing', function (st) {
		...
		forEach('f', function (item, index, list) {
